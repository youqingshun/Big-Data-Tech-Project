...
# try to use three models(DECISION TREE, RANDOMFOREST REGRESSOR, EXTRA TREE REGREESSOR) to fit the data
    def chooseModel(self, days, trafficDataPath):
        ...
        cars, bikes = dset(days, trafficDataPath)[0], dset(days, trafficDataPath)[1]
        xCars = cars.iloc[:, :-1]
        yCars =  cars.iloc[:, -1]
        xBikes = bikes.iloc[:, :-1]
        yBikes = bikes.iloc[:, -1]
...
# choose models respecitively
        ...
        columNames = ["Car Driving Rate", "Car Situation", "Bike Riding Rate", "Bike Situation"]
        ...
        plt.xticks(x, rowNames, rotation = 10)
        plt.ylim(0,1)
        plt.plot(x, yC)
        plt.plot(x, yB)
        plt.legend(labels = ['Cars', 'Bikes'], loc = "upper right")
        # print(res)
        plt.savefig('./static/d.png')
        plt.show()
        return res
