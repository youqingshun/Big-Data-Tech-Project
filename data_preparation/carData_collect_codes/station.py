...
class StationManager:
    ...
    STATION_FILE = "stations0.json"

    def __init__(self) -> None:
        if not os.path.isfile(self.STATION_FILE):
            with open("stations0.json", "w") as f:
                json.dump([], f)
    ...
    def list(self) -> List[Station]:
        with open("stations0.json", "r") as f:
...
class MySQLStationManager:
    ...
    def save(self, stations: List[Station]) -> None:
        ...
        query = "INSERT into parking (description, name, slotsTotal, slotsAvailable, monitored, parkAndRide, city, lat, lon, timestamp) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
